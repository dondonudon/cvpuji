...
class Stock_opname_model extends CI_Model
{
 ...
 {
  $hasil = $this->db->query("SELECT tab_barang.nama, temp_stock_opname.id_stock_opname, temp_stock_opname.nostockopname, temp_stock_opname.kode_barang, temp_stock_opname.stok, temp_stock_opname.datetime FROM temp_stock_opname INNER JOIN tab_barang ON tab_barang.kode_barang=temp_stock_opname.kode_barang");
  return $hasil->result();
 }

 public function simpan_barang($kode_barang, $stok, $nostockopname, $datetime)
 {
  $hasil = $this->db->query("INSERT INTO temp_stock_opname (nostockopname,kode_barang,stok,datetime)VALUES('$nostockopname','$kode_barang','$stok','$datetime')");
  return $hasil;
 }

 public function get_barang_by_kode($id_stock_opname)
 {
  $hsl = $this->db->query("SELECT tab_barang.nama, temp_stock_opname.id_stock_opname, temp_stock_opname.nostockopname, temp_stock_opname.kode_barang, temp_stock_opname.stok, temp_stock_opname.datetime FROM temp_stock_opname INNER JOIN tab_barang ON tab_barang.kode_barang=temp_stock_opname.kode_barang
 ...
 public function update_barang($stok, $id_stock_opname)
 {
  $hasil = $this->db->query("UPDATE temp_stock_opname SET stok='$stok' WHERE id_stock_opname='$id_stock_opname'");
  return $hasil;
 }
 ...
 public function insert_trans($notrans, $id_user, $ket, $datetime)
 {
  ...
  $q1 = $this->db->query("INSERT into stock_opname (nostockopname, ket, id_user, datetime) VALUES ('$notrans','$ket', '$id_user', '$datetime')");
  //insert into stock_opname_detail
  $q2 = $this->db->query("INSERT into stock_opname_detail (nostockopname, kode_barang, stok, datetime) SELECT nostockopname, kode_barang, stok, datetime FROM temp_stock_opname WHERE nostockopname = '$notrans'");
...
/* End of file Stock_opname_model.php */
/* Location: ./application/models/Stock_opname_model.php */
