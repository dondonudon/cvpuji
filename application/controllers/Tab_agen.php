...
class Tab_agen extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Tab_agen_model');
        $this->load->library('form_validation');        
        $this->session->set_flashdata('title', 'Master Agen | CV PUJI');   
	$this->load->library('datatables');
    }

    public function index()
    {
        $this->template->load('template','tab_agen/tbl_user_list');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->Tab_agen_model->json();
    }

    public function read($id) 
    {
        $row = $this->Agen_modle->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'is_aktif'      => $row->is_aktif,
            ...
            $this->template->load('template','tab_agen/tbl_user_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tab_agen'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button'        => 'Create',
            'action'        => site_url('tab_agen/create_action'),
        ...
        $this->template->load('template','tab_agen/tbl_user_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'is_aktif'      => $this->input->post('is_aktif',TRUE),
            ...
            $this->Tab_agen_model->insert($data);
    ...
    {
        $row = $this->Tab_agen_model->get_by_id($id);
    ...
    public function update_action() 
    {
		...
		'is_aktif'      => $this->input->post('is_aktif',TRUE));
            ...
            $this->Tab_agen_model->update($this->input->post('id_users', TRUE), $data);
	...
	foreach ($this->Tab_agen_model->get_all() as $data) {
            $kolombody = 0;
